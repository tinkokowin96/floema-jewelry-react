...
const About = ({ collections }) => {
  ...
  useEffect(() => {
    ...
    if (aboutState.history.action === "PUSH") {
    ...
    } else if (aboutState.history.action === "POP") {
  ...
  if (collections) {
    ...
    return (
      <div>
        {aboutState.history.action === "PUSH" && (
          <div className="flex flex-row fixed h-screen w-screen top-0 left-0 z-10">
            <div
              ref={(ele) => (transition.current[0] = ele)}
              className="flex-1 bg-red-500"></div>
            <div
              ref={(ele) => (transition.current[1] = ele)}
              className="flex-1 bg-green-500"></div>
            <div
              ref={(ele) => (transition.current[2] = ele)}
              className="flex-1 bg-blue-500"></div>
          </div>
        )}

        {aboutState.history.action === "POP" && (
          <div
            ref={firstLoad}
            className="bg-red-400 h-screen w-screen fixed top-0 left-0 z-10"></div>
        )}
                ...
                <div key={`${repeatSection.gallery}_${index}`}>
                  {collections.gallery.map((img: string, gal_ind: number) => (
                    <img
                      src={img}
                      alt=""
                      key={`galimg_${repeatSection.gallery}_${gal_ind}`}></img>
                  ))}
                </div>
              )
            }
                ...
                <div key={`${repeatSection.highlight}_${index}`}>
                  {section.items.map((img: any, hl_ind: number) => (
                    <img
                      src={img.image.url}
                      alt=""
                      key={`hlimg_${repeatSection.gallery}_${hl_ind}`}></img>
                  ))}
                  {title}
                  {label}
                </div>
